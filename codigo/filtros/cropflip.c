...
#define CROP_DEF(func_name) void func_name (unsigned char *src, unsigned char *dst, int cols, int filas, \
                      int src_row_size, int dst_row_size, int tamx, int tamy, int offsetx, int offsety)

CROP_DEF(cropflip_asm);
CROP_DEF(cropflip_asm2);
CROP_DEF(cropflip_asm3);
CROP_DEF(cropflip_c);
...
void aplicar_cropflip(configuracion_t *config)
{
	...
	if( config->tipo_filtro == FILTRO_ASM && config->version == 2) cropflip = cropflip_asm2;
	if( config->tipo_filtro == FILTRO_ASM && config->version == 3) cropflip = cropflip_asm3;
